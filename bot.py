...
# Telegram Bot Token
TOKEN = "7435691567:AAFka-SvGQTMhDvHi841HlcXr4bFb8U4dbM"
WEBHOOK_URL = "https://geschichtenkiosk.onrender.com"  # <- HIER DEINE DOMAIN
...
def webhook():
    ...
    if update.message:
        ...
        bot.send_message(chat_id=chat_id, text="Empfangen: " + message)
    return "ok", 200

@app.route('/set_webhook', methods=["GET"])
def set_webhook():
    if bot.set_webhook(url=f"{WEBHOOK_URL}/webhook"):
        return "Webhook gesetzt", 200
    else:
        return "Fehler beim Setzen des Webhooks", 500

@app.route('/remove_webhook', methods=["GET"])
def remove_webhook():
    if bot.remove_webhook():
        return "Webhook entfernt", 200
    else:
        return "Fehler beim Entfernen des Webhooks", 500
